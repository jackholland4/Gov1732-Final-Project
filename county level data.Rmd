...
```{r}
# Model 4: Turnout difference percentage
...
```{r}
labels <- add_lm_labels(ohio_joined_race_2, "inc_2022_BFA_Percap", "diff_margin")

ggplot(ohio_joined_race_2, aes(x = inc_2022_BFA_Percap, y = diff_margin)) +
  geom_jitter() +
...
ggplot(ohio_joined_race_2, aes(x = pct_female, y = diff_margin)) +
  ...
  labs(
    x = "% Female",
    y = "Difference in Margin (Abortion - Weed)",
    title = "Counties with a Larger Female Populations Lean More Pro-Abortion than Pro-Weed"
  ) +
  theme_minimal()
```
