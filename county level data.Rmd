...
inc_raw <- read_csv("Income.csv", show_col_types = FALSE)

inc_raw <- inc_raw %>%
  remove_empty("cols") %>%
  rename(label_grouping = `Label (Grouping)`) %>%
  select(-contains("Margin of Error"))   # <- drop all MOE columns


# Clean up the column names
names(inc_raw) <- names(inc_raw) %>%
  str_replace_all(",\\s*Ohio", "") %>%  # drop ", Ohio"
  str_replace_all("!!", " ") %>%        # replace "!!" with spaces
  str_squish()                          # trim/squish whitespace
```
...
inc_raw <- inc_raw %>%
  ...
  # plus any name-cleaning you already did on column names, if desired

# 1) Convert all non-label columns to numeric, treating ACS placeholders as NA
inc_num <- inc_raw %>%
  mutate(
    across(
      -label_grouping,
      ~ readr::parse_number(.x, na = c("NA", "(X)", "N/A", "-", "X"))
    )
  )

# 2) Get the totals from row 1 (now numeric)
totals <- inc_num %>%
  slice(1) %>%
  select(-label_grouping)

# 3) For rows 2–10, interpret values as percentages and convert to counts
inc_counts <- inc_num %>%
  mutate(row_id = row_number()) %>%
  mutate(
    across(
      -c(label_grouping, row_id),
      ~ if_else(
          row_id %in% 2:11 & !is.na(.) & !is.na(totals[[cur_column()]]),
          round(. / 100 * totals[[cur_column()]]),
          .    # leave row 1 and any other rows as they are (or NA)
        )
    )
  ) %>%
  select(-row_id)

names(inc_counts) <- names(inc_counts) %>% 
  # remove the ", Ohio"
  str_replace_all(",\\s*Ohio", "") %>%
  # replace "!!" with a space
  str_replace_all("!!", " ") %>%
  # drop the trailing "Estimate"
  str_replace("\\s*Estimate$", "") %>%
...
# 1) Get all non-label columns and split them into groups of 4
all_cols  <- names(inc_counts)
value_cols <- all_cols[all_cols != "label_grouping"]

groups <- split(value_cols, ceiling(seq_along(value_cols) / 4))

# 2) For each group of 4, create a new "X Sums" column = row-wise sum of those 4
for (g in groups) {
  last_col  <- g[length(g)]
  first_word <- str_extract(last_col, "^[^\\s]+")  # first word of the last column name
  new_name  <- paste(first_word, "Sums")

  inc_counts[[new_name]] <- rowSums(inc_counts[g], na.rm = TRUE)
}

# 3) Optional: reorder so each Sums column comes right after its 4 originals
new_order <- c("label_grouping")
for (g in groups) {
  last_col  <- g[length(g)]
  first_word <- str_extract(last_col, "^[^\\s]+")
  sum_col   <- paste(first_word, "Sums")
  new_order <- c(new_order, g, sum_col)
}

inc_counts <- inc_counts[, new_order]
...
keep_cols <- c(
  "label_grouping",
  grep("Sums", names(inc_counts), value = TRUE, ignore.case = TRUE)
)

inc_counts <- inc_counts[, keep_cols]
```
...
names(inc_counts) <- names(inc_counts) |>
  str_remove(regex("sums", ignore_case = TRUE)) |>
  str_squish()
```